...
async function reading(type){
    //看文章或者视频
    let time = parseInt(Math.random()*(90-65+1)+65,10) ;//65-90秒后关闭页面
    // let firstTime = time - 2;
    // let secendTime = 12;
    let firstTime = Math.floor(Math.random() * 4) + (time - 5); // time-5 到 time-2 随机值
    let secendTime = Math.floor(Math.random() * 5) + 8; // 8-12 随机值
